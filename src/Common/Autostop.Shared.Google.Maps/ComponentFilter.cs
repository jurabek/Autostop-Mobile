...
namespace Google.Maps
{
    ...
    ///     The restriction is specified using the components filter. A filter consists of a list of component:value pairs
    ///     separated
    ///     by a pipe (|). Only the results that match all the filters will be returned. Filter values support the same methods
    ///     of
    ///     spelling correction and partial matching as other geocoding requests. If a geocoding result is a partial match for
    ///     a
    ...
    public class ComponentFilter
    {
        ...
        {
            get => TryGetParameter("administrative_area");
        ...
        {
            get => TryGetParameter("postal_code");
        ...
        {
            var parametersList = parameters.Where(p => !(string.IsNullOrEmpty(p.Value) || p.Value.Trim().Length == 0))
        ...
        ///     A filter consists of a list of component:value pairs separated by a pipe (|). For exemple:
        ///     \"country:ES|locality:Santa Cruz de Tenerife\".
        /// </param>
        /// <returns></returns>
            ...
            foreach (var filter in filters)
            {
                ...
                if (filterSplitted.Length != 2)
                    throw new ArgumentException(
                        "One of the filters is not well formated. Make sure that each filter is written \"component:value\" and are separated by \"|\". For exemple, \"country:ES|locality:Santa Cruz de Tenerife\"",
                        "components");
                ...
                switch (component.ToLowerInvariant())
                {
                    ...
                    case "country":
                        ...
                        break;
                    default:
                        throw new ArgumentException("The value '" + component + "' is not a valid component.",
                            "components");
                }
            }
