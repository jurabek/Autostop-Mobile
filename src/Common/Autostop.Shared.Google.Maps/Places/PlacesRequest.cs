...
namespace Google.Maps.Places
{
    public abstract class PlacesRequest : BaseRequest
    {
        ...
        {
            if (Minprice.HasValue && (Minprice < 0 || Minprice > 4))
                throw new ArgumentException("Minprice property must have a value between 0 and 4 inclusive.");

            if (Maxprice.HasValue && (Maxprice < 0 || Maxprice > 4))
                throw new ArgumentException("Maxprice property must have a value between 0 and 4 inclusive.");
        ...
        {
            return string.Join("|", Types.Select(t => t.ToString().ToLowerInvariant()).ToArray());
        }
    }
}