...
namespace Google.Maps.Places.Autocomplete
{
    public class AutocompleteRequest : BaseRequest
    {
        ...
        public override Uri ToUri()
        {
            ...
            var url = "autocomplete/json?" + qsb;
            return new Uri(url, UriKind.Relative);
        }

        protected void ValidateRequest()
        {
            if (string.IsNullOrEmpty(Input)) throw new InvalidOperationException("Input property hasn't been set.");
        }

        protected string TypesToUri()
        {
            return string.Join("|",
                Types.Select(t => ReplaceCamelCaseToUnderscore(t.ToString()).ToLowerInvariant()).ToArray());
        }

        protected string ReplaceCamelCaseToUnderscore(string input)
        {
            return string.Concat(input.Select((x, i) =>
                i > 0 && char.IsUpper(x) || char.IsNumber(x) ? "_" + x.ToString() : x.ToString()));
        }
    }
}